 ...
 * (zephyrproject-rtos/zephyr#111009) but re-homed onto the role-neutral SDIO
 ...
 * reserved for @ref k_fifo bookkeeping.
 */
struct sdio_pkt {
 ...
 * @ref sdio_stream_function_init, then register @ref base with
 ...
 *
 * Sets up @ref base with a FIFO data port at @p fifo_reg and installs the
 * streaming FIFO handler, and initializes the RX/TX FIFOs. The caller
 * registers @ref base with @ref sdio_device_register_function.
 *
